...
pub use crypto::keys::{
    KeyPair, generate_keypair_from_mnemonic, generate_new_keypair_with_mnemonic,
};
...
impl fmt::Display for KeyGenConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "KeyGenConfig {{ output_path: {:?}, comment: {:?}, passphrase: [redacted], mnemonic_length: {:?}, mnemonic_file: {:?} }}",
