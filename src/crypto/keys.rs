...
impl KeyPair {
    ...
    fn new(signing_key: SigningKey, comment: Option<&str>, passphrase: Option<&str>) -> Result<Self> {
...
impl fmt::Debug for KeyPair {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "KeyPair {{ verifying_key: {:?}, private_key: [REDACTED], public_key: {:?} }}",
...
fn format_openssh_private_key(signing_key: &SigningKey, passphrase: Option<&str>) -> Result<String> {
    ...
    let cipher_name = if passphrase.is_some() { "aes256-ctr" } else { "none" };
    buffer.extend_from_slice(&(cipher_name.len() as u32).to_be_bytes());
    buffer.extend_from_slice(cipher_name.as_bytes());
    
    // KDF (key derivation function)
    let kdf_name = if passphrase.is_some() { "bcrypt" } else { "none" };
