...
fn test_generate_keypair_from_mnemonic() {
    ...
    assert!(keypair.private_key_openssh().contains("-----BEGIN OPENSSH PRIVATE KEY-----"));
    assert!(keypair.private_key_openssh().contains("-----END OPENSSH PRIVATE KEY-----"));
}
...
fn test_generate_new_keypair_with_mnemonic() {
    ...
    let (mnemonic, keypair) = generate_new_keypair_with_mnemonic(length, comment, passphrase).unwrap();
...
fn test_verify_key_with_mnemonic() {
    ...
    let different_keypair = generate_keypair_from_mnemonic(&different_mnemonic, None, None).unwrap();
